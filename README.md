...
- `pages` — на сколько картинок резать отчёт и что на какой показывать (`sections` по id, флаги `hero`, `capacity`, `footnote`). Все картинки уходят одним сообщением в Slack. Убрать `pages` — будет одна длинная картинка. Сноска «Что здесь реальное» по умолчанию выключена, включается `"footnote": true` у страницы.
