...
- `pages` — на сколько картинок резать отчёт и что на какой показывать (`sections` по id, флаги `hero`, `capacity`, `footnote`). Все картинки уходят одним сообщением в Slack. Убрать `pages` — будет одна длинная картинка.
